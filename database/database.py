...
class Database:
    ...
    def salvar_leitura(cls, unidade, agua, gas, tipo=None, permitir_forcagem=False):
        ...
        agora = datetime.now()
        data_hora_txt = agora.strftime("%Y-%m-%d %H:%M:%S") # Correto ✅
        hoje = datetime.now().strftime("%Y-%m-%d")          # Correto ✅
        max_retries = 3
        ...
        try:
            with cls.get_db() as conn:
                ...
                for row in reversed(rows):
                    # Correção: extrair a data do banco com segurança
                    dt_str = row['data_leitura'][:10] # Pega YYYY-MM-DD
                    historico.append({"mes": dt_str, "valor": row['leitura_agua']})
